...
export default class RouterListener {
  ...
  attach({ router, siteAnalytics, screenReaderSupport }: Dependencies) {
    ...
    (router as any).onRouteChangeStart = this.routeChangeStart;
    (router as any).onRouteChangeComplete = this.routeChangeComplete;
    (router as any).onRouteChangeError = this.routeChangeError;
  ...
  detach() {
    if (this.router) {
      (this.router as any).onRouteChangeStart = null;
      (this.router as any).onRouteChangeComplete = null;
      (this.router as any).onRouteChangeError = null;
    }
